...
tree-sitter の markdown パーサーは、連続する > の行に対して 最初の行にしか block_quote_marker ノードを生成しません。2行目以降の > は AST ノードとして存在しないため、現在の find_and_render_markers による再帰探索では最初の行しか処理できていません。
ネストの場合も同様で、外側の block_quote が内側の > を二重に拾おうとする挙動になっています。

修正方法としてfind_and_render_markers による AST ノード探索をやめ、block_quote ノードの行範囲を直接イテレートして各行の > を文字列として探す方式に変えます。
ネストされた block_quote も query:iter_captures でそれぞれ個別にキャプチャされるので、各ノードは自分の start_col から始まる > だけを担当させれば二重処理も防げます。
