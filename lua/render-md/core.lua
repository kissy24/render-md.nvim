    ...
    for id, node, _ in query:iter_captures(root, bufnr, 0, -1) do
            ...
            for i = 0, node:child_count() - 1 do
                ...
                if c_type == "-" or c_type == "*" or c_type == "+" or c_type:find("marker") then
                    ...
                    if next_child and next_child:type():find("task_list_marker") then
                        is_task = true
                        local icon = next_child:type():find("unchecked") and config.icons.unchecked or
                        config.icons.checked
        ...
        elseif capture_name == "quote" then
            for row = start_row, end_row - 1 do
                ...
                if not line then goto next_capture end

                -- このノードの start_col 以降で最初の > を探す
                local col = start_col
                -- インデント空白をスキップ
